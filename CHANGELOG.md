...
- Added `readme-echo check --heading-counts` for optional text-only source and checked-target heading totals after ignore and depth filters.
...
- Added `readme-echo check --require-targets` to fail CI when no effective target README files are configured or discovered.
